...
(function initLoader() {
  ...
  /* ── pixel-art CROWZ that assembles block by block ── */
  const GLYPHS = {
  ...
  const PIX_WORD = ['C', 'R', 'O', 'W', 'Z'];
  const pixCanvas = document.getElementById('loaderPixels');

  (function pixelWord() {
    if (!pixCanvas || !pixCanvas.getContext) return;
    const pctx = pixCanvas.getContext('2d');
    const rows = GLYPHS.C.length;
    const letterW = 5, letterGap = 1, cellGap = 2;
    const cols = PIX_WORD.length * letterW + (PIX_WORD.length - 1) * letterGap;
    const cell = Math.max(7, Math.min(16, Math.floor(Math.min(window.innerWidth * .88, 560) / 29) - 2));
    const pitch = cell + cellGap;
    const W = cols * pitch - cellGap;
    const H = rows * pitch - cellGap;
    const dpr = Math.min(devicePixelRatio || 1, 2);
    pixCanvas.width = W * dpr; pixCanvas.height = H * dpr;
    pixCanvas.style.width = W + 'px'; pixCanvas.style.height = H + 'px';
    pctx.setTransform(dpr, 0, 0, dpr, 0, 0);

    const cells = [];
    PIX_WORD.forEach((ch, li) => {
      const glyph = GLYPHS[ch];
      glyph.forEach((rowStr, r) => {
        for (let c = 0; c < letterW; c++) {
          if (rowStr[c] === '1') {
            cells.push({
              x: (li * (letterW + letterGap) + c) * pitch,
              y: r * pitch,
              s: cell,
              ord: r + c + li * 3 + Math.random() * .6,
              hot: ch === 'Z'
            });
          }
        }
    ...
    function draw(now) {
      ...
      pctx.fillStyle = 'rgba(255,255,255,.045)';
      for (const c of cells) pctx.fillRect(c.x, c.y, c.s, c.s);
      const N = cells.length;
      for (let i = 0; i < N; i++) {
        const c = cells[i];
        const local = (t * N - i) * 1.7;
        if (local < 0 || local > 1) continue;
        const sc = .25 + .75 * easeOutCubic(local);
        const pad = c.s * (1 - sc) / 2;
        pctx.fillStyle = c.hot ? 'rgba(252,165,165,1)' : 'rgba(220,38,38,1)';
        pctx.fillRect(c.x + pad, c.y + pad, c.s * sc, c.s * sc);
      }
      if (t < 1 && !reduced) requestAnimationFrame(draw);
    }
